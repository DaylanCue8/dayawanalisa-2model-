...
# small default straight to Dot instead of risking a false X/Cross
...
def classify_glyph(native_crop, base_model, dia_model, base_classes, dia_classes,
    ...
    if dia_idx is None:
        ...
        # from multiple disconnected pieces of the same stroke).
        base_mask = np.isin(labels, base_idx_list).astype(np.uint8) * 255
        coords = cv2.findNonZero(base_mask)
        bx, by, bw, bh = cv2.boundingRect(coords)
        base_crop = base_mask[by:by + bh, bx:bx + bw]
        ...
        # out rather than amplified into hard, artificial notches - a
        # genuine dot at very low resolution was previously reading as
        # "concave enough to be an X" purely from thresholding noise.
        ...
        # cv2.minAreaRect finds the tightest rectangle at ANY angle, so a
        # curved or diagonally-drawn dash still measures as "long and
        # thin" along its own natural axis, instead of being judged
        # against a horizontal/vertical bounding box (which penalizes
        # curved strokes and can misread them as low-solidity "X" marks).
        ...
        if debug:
            print(f"  [debug] dw={dw}, dh={dh}, area={dw * dh}, "
                  ...
                  f"svm_pred='{svm_raw_dia_prediction}', position='{position}'")

        # ---- MINIMUM-SIZE GUARD ----
        # A mark this small doesn't have enough pixels for solidity or
        # aspect-ratio measurements to reliably tell shapes apart - a
        # few-pixel dot can look "concave" purely from binarization
        # jaggedness. Default straight to Dot rather than risking a
        # false Bar/X override on noise. This check runs BEFORE the bar
        # check too, since a tiny mark's rotated aspect ratio is just as
        # unreliable as its solidity.
        ...
        if debug and predicted_dia_name != svm_raw_dia_prediction:
            print(f"  [debug] OVERRIDE applied: svm said '{svm_raw_dia_prediction}' "
                  f"-> geometry forced '{predicted_dia_name}'")
